...
ggsave("Write_up/figures/predicted_valence_aftIND_aftPM_PM.png",
...
# now we want to know whether it is positive or negative side that drives the effect
long_df_merged$post_ind_pos_spline_nozero<- ifelse(long_df_merged$valence_aftIND_min_bef==0,NA, long_df_merged$post_ind_pos_spline )
long_df_merged$post_ind_neg_spline_nozero<- ifelse(long_df_merged$valence_aftIND_min_bef==0,NA,long_df_merged$post_ind_neg_spline )

long_df_merged$post_PM_pos_spline_no_zero<- ifelse(long_df_merged$aft_PM_minus_after_ind==0,NA, long_df_merged$post_PM_pos_spline)
long_df_merged$post_PM_neg_spline_no_zero<- ifelse(long_df_merged$aft_PM_minus_after_ind==0,NA, long_df_merged$post_PM_neg_spline )
...
long_df_ar_merged$PM_task_lenient_av_log<-car::logit(long_df_ar_merged$PM_task_lenient_av, percents = F, adjust = minimum )

reg_ar<-lmer(PM_task_lenient_av_log~ aft_PM_minus_after_ind*agegroup.c + 
            arousal_aftIND_min_bef*agegroup.c+(1|participant), 
          data = long_df_ar_merged[ (is.na(long_df_ar_merged$MOCA)|long_df_ar_merged$MOCA>=26),])
...
long_df_merged_val_ar<-merge(long_df_merged,
                             long_df_ar_merged[, c(
                                                   ...
                                                   "arousal_aftIND_min_bef.s")], by = c("participant", "cond"))
...
reg_ar<-lmer(PM_task_lenient_av_log~ aft_PM_minus_after_ind_ar.s*agegroup.c + 
             ...
             data = long_df_merged_val_ar[ (is.na(long_df_merged_val_ar$MOCA)|long_df_merged_val_ar$MOCA>=26),])
